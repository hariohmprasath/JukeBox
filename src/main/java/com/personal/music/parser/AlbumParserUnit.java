...
public class AlbumParserUnit implements Callable<AlbumParserResponse> {
            ...
            if (albumList != null && albumList.size() > 0) {
                for (AlbumJSON albumJSON : albumList) {
                    ...
                    if (!albumNameCollection.contains(albumIndexName)) {
                        getSongs(albumCollection, albumJSON, albumIndexName);
                    } else {
    ...
    public void getSongs(List<AlbumJSON> albumCollection, AlbumJSON albumJSON, String albumIndexName) throws Exception {
        Document doc = Jsoup.connect(albumJSON.getAlbumUrl()).get();
        String imageUrl = null;
        if (doc != null) {
            // Get thumbnail URL for preview
            String imagePath = albumParserConfiguration.getAlbumReaderConfiguration().getSiteImage();
            String path[];
            if (imagePath != null && imagePath.trim().length() > 0 && imagePath.contains(","))
                path = imagePath.split(",");
            else
                path = new String[]{imagePath};

            for (String imgPath : path) {
                imageUrl = Utils.getDataFromHtmlNode(doc.getAllElements().get(0), imgPath);
                if (imageUrl != null && imageUrl.trim().length() > 0)
                    break;
            }

            for (SongReaderConfiguration songReaderConfiguration : albumParserConfiguration.getSongReaderConfiguration()) {
                String key = StringEscapeUtils.unescapeXml(songReaderConfiguration.getList());
                ...
                if (listOfElements != null && listOfElements.size() > 0) {
                    ...
                    for (Element songItem : listOfElements) {
                        String songName = Utils.getDataFromHtmlNode(songItem, songReaderConfiguration.getName());
                        songName = getExtractMediaName(songReaderConfiguration, songName);

                        String songUrl = Utils.getDataFromHtmlNode(songItem, songReaderConfiguration.getUrl());

                        // Check whether its valid endsWith token
                        if (songUrl != null) {
                            boolean isValid = true;
                            if (songUrl.length() == 0 || (songReaderConfiguration.getEndsWith() != null && !songUrl.endsWith(songReaderConfiguration.getEndsWith())))
                                isValid = false;
                    ...
                    // Check whether songs are available or else move to next configuration
